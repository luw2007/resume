...
impl Scope {
    ...
    ///
    /// Per `docs/product-design.md` Â§4 ("Git metadata performance"): "Query
    /// each normalized Workspace at most once" and cache only for the current
    /// process. `git_common_dir` is a `git rev-parse` subprocess spawn, so two
    /// optimizations apply:
    ///
    /// 1. It is computed only in `ScopeMode::Git`, the only mode that reads
    ///    it (`contains` ignores `git_common_dir` in every other mode) â
...
mod tests {
    ...
    /// `docs/product-design.md` Â§4 ("Git metadata performance"): "Query each
    /// normalized Workspace at most once." `contains_workspace` spawns
    /// `git rev-parse` per call in `ScopeMode::Git`; without memoization,
    /// looking up the same canonical path many times costs one subprocess
    /// spawn each time (multiple ms each). This asserts the *second* batch of
    /// identical-path calls is not asymptotically as expensive as the first
    /// â a regression to "spawn every time" would make both batches equally
    /// slow, while a correctly memoized cache makes the second batch orders
    /// of magnitude faster. Uses a generous 5x threshold (not raw ms) to stay
    /// robust across slow/loaded CI runners while still catching a
    /// reintroduced per-call spawn.
    #[test]
