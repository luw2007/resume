...
function __fish_resume_global_optspecs
    string join \n U/up= D/down= a/agent= since= list json verbose config= confirm-always no-confirm man h/help V/version
end
...
complete -c resume -n "__fish_resume_using_subcommand help; and not __fish_seen_subcommand_from config completions help" -f -a "config" -d 'Inspect resume configuration'
complete -c resume -n "__fish_resume_using_subcommand help; and not __fish_seen_subcommand_from config completions help" -f -a "completions" -d 'Print a shell completion script to stdout'
