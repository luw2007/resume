...
//!    (`docs/product-design.md` Â§4: "Query each normalized Workspace at
//!    most once").
...
//!    (`docs/product-design.md` Â§3: "at most a 1 MiB bounded early read").
//!
//! ## Fixtures
//!
...
//! ```
//!
//! `criterion` writes HTML reports under `target/criterion/` (gitignored)
//! when the `html_reports` feature is enabled; this crate depends on the
//! lighter `cargo_bench_support` feature only, so reports are text-only by
//! default to keep the dev-dependency footprint small per
//! `plans/v0.1.0-implementation.md`'s "minimal dependency set" guidance.
