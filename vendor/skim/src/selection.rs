...
/// The list reserves each item's own rendered rows plus a separator. Kept in
/// `SkimItem::display_height`, so title+message cards can be taller than
/// summary-only cards without wasting rows or breaking navigation.
...
impl Selection {
    ...
    pub fn act_move_line_cursor(&mut self, diff: i32) {
        ...
        let next = (current as i32 + diff).clamp(0, self.items.len().saturating_sub(1) as i32) as usize;
        self.item_cursor = self.item_cursor.min(next);
        self.line_cursor = next - self.item_cursor;
        self.fit_cursor();
    }

    pub fn act_select_screen_row(&mut self, rows_to_top: usize) {
        let capacity = self.item_capacity();
        let total: usize = (self.item_cursor..self.item_cursor + capacity).map(|i| self.item_height(i)).sum();
        let mut row = if self.reverse { 0 } else { self.height.load(Ordering::Relaxed).saturating_sub(total) };
    ...
    fn item_height(&self, index: usize) -> usize {
        self.items.get(index).map_or(2, |item| item.item.display_height().max(2))
    }
    ...
    fn fit_cursor(&mut self) {
        ...
        let selected = (self.item_cursor + self.line_cursor).min(self.items.len() - 1);
        self.item_cursor = self.item_cursor.min(selected);
        let height = self.height.load(Ordering::Relaxed).max(1);
        while (self.item_cursor..=selected).map(|i| self.item_height(i)).sum::<usize>() > height && self.item_cursor < selected {
            self.item_cursor += 1;
        }
        self.line_cursor = selected - self.item_cursor;
    }

    fn item_capacity(&self) -> usize {
        let height = self.height.load(Ordering::Relaxed).max(1);
        let mut used = 0;
        let mut index = self.item_cursor;
        while index < self.items.len() && used + self.item_height(index) <= height {
            used += self.item_height(index);
            index += 1;
        }
        (index - self.item_cursor).max(1)
    }
}
    ...
    fn handle(&mut self, event: &Event) -> UpdateScreen {
        ...
        match event {
            ...
            EvActHalfPageDown(diff) => {
                let height = 1 - (self.item_capacity() as i32);
                self.act_move_line_cursor(height * *diff / 2);
            }
            EvActHalfPageUp(diff) => {
                let height = (self.item_capacity() as i32) - 1;
                self.act_move_line_cursor(height * *diff / 2);
            }
            EvActPageDown(diff) => {
                let height = 1 - (self.item_capacity() as i32);
...
impl Selection {
    fn draw_item(
        ...
        let matches = match &matched_item.matched_range {
            Some(MatchRange::Chars(matched_indices)) => Matches::CharIndices(matched_indices),
        ...
        let mut display_rows = item.display_rows(context);

        if display_rows.len() == 1 {
            // Single-row item: preserve the original highlight-aware path
            // exactly, including fuzzy-match shift/highlight rendering.
            let display_content = display_rows.pop().unwrap();
        ...
        // Multi-row card: each row prints independently with its own width,
        // no fuzzy-match highlight shift (the flat `item_text` used for
        // matching is not expected to equal any single card row).
        display_rows.truncate(CARD_TEXT_ROWS);
        for (offset, content) in display_rows.into_iter().enumerate() {
            let mut printer = LinePrinter::builder()
                .row(row + offset)
                .col(2)
                .tabstop(self.tabstop)
                .container_width(container_width)
                .text_width(content.stripped().width_cjk())
                .hscroll_offset(self.hscroll_offset)
                .build();
            print_item(canvas, &mut printer, content, default_attr);
        }
...
impl Draw for Selection {
    fn draw(&self, canvas: &mut dyn Canvas) -> DrawResult<()> {
        ...
        let item_idx_upper = min(self.item_cursor + capacity, self.items.len());

        clear_canvas(canvas)?;

        let total_height: usize = (item_idx_lower..item_idx_upper).map(|i| self.item_height(i)).sum();
        let mut row = if self.reverse { 0 } else { screen_height.saturating_sub(total_height) };
        for item_idx in item_idx_lower..item_idx_upper {
            let line_cursor = item_idx - item_idx_lower;
            let row_block = row;
            row += self.item_height(item_idx);
            ...
            let _ = self.draw_item(canvas, row_block, &item, line_cursor == self.line_cursor);
        }
