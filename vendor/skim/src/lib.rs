...
pub trait SkimItem: AsAny + Send + Sync + 'static {
    ...
    /// The content to be displayed on the item list as a stacked, multi-row
    /// card. Default delegates to `display()` for a single row; the list
    /// widget pads/truncates to its fixed row-per-item stride.
    fn display_rows<'a>(&'a self, context: DisplayContext<'a>) -> Vec<AnsiString<'a>> {
        vec![self.display(context)]
    }

    /// Terminal rows occupied by this item, including its separator.
