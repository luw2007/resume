...
distinct Workspaces). This document explains the benchmark suite added to measure and
guard against that class of regression, and the two root causes it was built to isolate.
...
call, even when many Sessions share the same Workspace â the common case for a
...
redundant subprocess spawns for the other ~3000 calls.

Fixed by:

1. Only computing `git_common_dir` in `ScopeMode::Git` â `contains()` never reads it in
...
2. Memoizing the result per canonical path in a `Scope`-owned `Mutex<HashMap<..>>` (the
   ...
   sound here), per `docs/product-design.md` Â§4: "Query each normalized Workspace at
   most once ... Cache only for the current process."

Measured effect (manual A/B comparison against a real Git repo, 1000 calls across 20
unique Workspace paths): **5.6ms/call unpatched â ~0.02ms/call patched at scale**
...
`session_meta` record and derive a title from user messages â even for outlier files tens
of megabytes large. `docs/product-design.md` Â§3 specifies title derivation should use "at
most a 1 MiB bounded early read," which the current implementation does not honor for
Codex. The `uniform_*files_*lines` vs `uniform_*_plus_NxMB` benchmark pair makes this
